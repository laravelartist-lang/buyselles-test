...
import 'package:sixvalley_vendor_app/features/splash/domain/models/config_model.dart' hide Data;
...
class ShopController extends ChangeNotifier {
  ...
  void choosePhoto() async {
    final pickedFile = await picker.pickImage(source: ImageSource.gallery, imageQuality: AppConstants.imageQuality, maxHeight: 500, maxWidth: 500);
    if (pickedFile != null) {
  ...
  void pickTinCertificateFile() async {
    FilePickerResult? result = await FilePicker.platform.pickFiles(
      ...
      withReadStream: true,
      withData: true,
      allowedExtensions: ['pdf', 'jpg', 'jpeg', "doc", 'docx'],
    );

    bool hasInvalidFile = false;
    if(result != null) {
      if (['pdf', 'jpg', 'jpeg', "doc", 'docx'].contains(result.files.first.extension?.toLowerCase())) {
      } else {
        hasInvalidFile = true;
        result = null;
      }
    ...
    if(result != null){
      ...
      double fileSizeInMB = await ImageValidationHelper.getImageSizeFromXFile(xFile);
  ...
  List<LocationItemModel> _countries = [];
  List<LocationItemModel> get countries => _countries;

  List<LocationItemModel> _cities = [];
  List<LocationItemModel> get cities => _cities;

  List<LocationItemModel> _areas = [];
  List<LocationItemModel> get areas => _areas;
  ...
  bool get isLocationSaving => _isLocationSaving;

  void initShopLocationFromModel() {
    _selectedCountryId = _shopModel?.storeCountryId;
    _selectedCityId = _shopModel?.storeCityId;
    _selectedAreaId = _shopModel?.storeAreaId;
  }

  Future<void> fetchCountries({String? search}) async {
    _isLocationLoading = true;
    notifyListeners();
    final ApiResponse response = await shopServiceInterface.getCountries(search: search);
    if (response.response?.statusCode == 200) {
      _countries = [];
      for (final item in response.response!.data as List) {
        _countries.add(LocationItemModel.fromJson(item));
      }
    } else {
  ...
  Future<void> fetchCities(int countryId, {String? search}) async {
    _isLocationLoading = true;
    notifyListeners();
    final ApiResponse response = await shopServiceInterface.getCities(countryId, search: search);
    if (response.response?.statusCode == 200) {
      _cities = [];
      for (final item in response.response!.data as List) {
        _cities.add(LocationItemModel.fromJson(item));
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> fetchAreas(int cityId, {String? search}) async {
    _isLocationLoading = true;
    notifyListeners();
    final ApiResponse response = await shopServiceInterface.getAreas(cityId, search: search);
    if (response.response?.statusCode == 200) {
      _areas = [];
      for (final item in response.response!.data as List) {
        _areas.add(LocationItemModel.fromJson(item));
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<bool> saveShopLocation() async {
    if (_shopModel == null || _selectedCountryId == null) {
      showCustomSnackBarWidget(
        getTranslated('select_country', Get.context!) ?? 'Please select a country',
        Get.context!,
      );
      return false;
    }
    ...
    final ApiResponse response = await shopServiceInterface.updateShopLocation(
      _shopModel!,
    ...
    if (response.response?.statusCode == 200) {
      showCustomSnackBarWidget(
        getTranslated('updated_successfully', Get.context!) ?? 'Updated successfully',
        Get.context!,
        isError: false,
      );
      await getShopInfo();
