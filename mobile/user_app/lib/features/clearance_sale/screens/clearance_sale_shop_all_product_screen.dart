...
class ClearanceSaleShopProductScreen extends StatefulWidget {
  final String slug;
  const ClearanceSaleShopProductScreen({super.key, required this.slug});
                ...
                child: Consumer<ShopController>(
                    builder: (context, productController, child) {
                      return ClearanceSearchBarWidget(
                        ...
                        onSearchClick: () => onClick(context, productController, _searchController, widget.slug),
                        searchIcon: Icon(!productController.isSearchActive ? Icons.search : Icons.close, color: Theme.of(context).cardColor, size: 15),
                        applyFilter: () => applyShopFilter(context: context, sellerId: widget.slug),
                      );
                })
            ),
                  ...
                  return (productModel != null && !shopController.isSearchLoading)
                      ...
                      ? PaginatedListView(scrollController: _scrollController,
                        onPaginate: (int? offset) async => (shopController.isSearchActive || shopController.isFilterActive)
                            ? applyShopFilter(context: context, sellerId: widget.slug, fromPagination: true, offset: offset)
                            : await shopController.getClearanceShopProductList('clearance_sale', offset.toString(), widget.slug),
                      ...
                      ) : NoInternetOrDataScreenWidget(isNoInternet: false, icon: Images.noProduct, message: 'no_product_found') : ProductShimmer(isEnabled: productModel == null, isHomePage: false);
                  }),
                ),
              ),
            ),
...
void onClick( BuildContext context, ShopController shopController, TextEditingController searchController, String sellerId){
  if(searchController.text.isEmpty && !shopController.isSearchActive) {
    showCustomSnackBarWidget(getTranslated('type_something_to_search_for_products', context), context, snackBarType: SnackBarType.warning);
