...
class SearchProductController with ChangeNotifier {
  ...
  void setFilterIndex(int index) {
    ...
    if(index == 0){
      ...
      sortText = 'rating-low-high';
    }else if(index == 7){
      sortText = 'rating-high-low';
    }
    notifyListeners();
  }
  ...
  Future searchProduct({required String query, String? categoryIds, String? brandIds,  String? authorIds, String? publishingIds, String? sort, String? priceMin, String? priceMax, required int offset, int? countryId, int? cityId, int? areaId}) async {
    if(query.isNotEmpty){
    ...
    ApiResponseModel apiResponse = await searchProductServiceInterface!.getSearchProductList(query, categoryIds, brandIds, authorIds, publishingIds, sort, priceMin, priceMax, offset, _productTypeIndex == 0 ? 'all' : _productTypeIndex == 1 ? 'physical' : 'digital', countryId: countryId, cityId: cityId, areaId: areaId);
  ...
  /// True only when [_countries] was loaded without a search filter (full list).
  /// Prevents re-fetching the full list on every dialog open while still
  /// allowing a reload whenever the list was narrowed by a previous search.
  bool _countriesLoadedFull = false;

  Future<void> getCountries({String? search}) async {
    final isEmptySearch = search == null || search.isEmpty;

    // Only skip a network call if the full, unfiltered list is already in memory.
    if (_countriesLoadedFull && isEmptySearch) {
      return;
    }
    ...
    if (!isEmptySearch) {
      ...
      _locationDebounce = Timer(const Duration(milliseconds: 500), () async {
        ApiResponseModel apiResponse =
            await searchProductServiceInterface!.getCountries(search: search);
        if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
          _countries = [];
          apiResponse.response!.data
              .forEach((country) => _countries!.add(CountryModel.fromJson(country)));
          _countriesLoadedFull = false; // filtered result — not the full list
          notifyListeners();
        }
      });
    } else {
      ApiResponseModel apiResponse = await searchProductServiceInterface!.getCountries();
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        _countries = [];
        apiResponse.response!.data
            .forEach((country) => _countries!.add(CountryModel.fromJson(country)));
        _countriesLoadedFull = true;
        notifyListeners();
      } else {
        ApiChecker.checkApi(apiResponse);
      }
  ...
  Future<void> getCities(int countryId, {String? search}) async {
    final isSearch = search != null && search.isNotEmpty;
    if (isSearch) {
      if (_locationDebounce?.isActive ?? false) _locationDebounce!.cancel();
      _locationDebounce = Timer(const Duration(milliseconds: 500), () async {
        ApiResponseModel apiResponse =
            await searchProductServiceInterface!.getCities(countryId, search: search);
        if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
          _cities = [];
          apiResponse.response!.data.forEach((city) => _cities!.add(CityModel.fromJson(city)));
          notifyListeners();
        }
      });
    } else {
      // Full reload — clear immediately so the UI shows a loading state
      // rather than the previous search-filtered subset.
      _cities = null;
      notifyListeners();
      ApiResponseModel apiResponse =
          await searchProductServiceInterface!.getCities(countryId);
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        _cities = [];
        apiResponse.response!.data.forEach((city) => _cities!.add(CityModel.fromJson(city)));
        notifyListeners();
  ...
  Future<void> getAreas(int cityId, {String? search}) async {
    final isSearch = search != null && search.isNotEmpty;
    if (isSearch) {
      if (_locationDebounce?.isActive ?? false) _locationDebounce!.cancel();
      _locationDebounce = Timer(const Duration(milliseconds: 500), () async {
        ApiResponseModel apiResponse =
            await searchProductServiceInterface!.getAreas(cityId, search: search);
        if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
          _areas = [];
          apiResponse.response!.data.forEach((area) => _areas!.add(AreaModel.fromJson(area)));
          notifyListeners();
        }
      });
    } else {
      // Full reload — clear immediately.
      _areas = null;
      notifyListeners();
      ApiResponseModel apiResponse =
          await searchProductServiceInterface!.getAreas(cityId);
      if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
        _areas = [];
        apiResponse.response!.data.forEach((area) => _areas!.add(AreaModel.fromJson(area)));
        notifyListeners();
  ...
  void setSelectedCountry(int? id) {
  ...
  void resetLocationFilter() {
    ...
    _cities = null;
    _areas = null;
    // Clear the cache flag so the next getCountries() call always
    // fetches the complete, unfiltered list from the server.
    _countriesLoadedFull = false;
    getCountries();
    notifyListeners();
  }
}
