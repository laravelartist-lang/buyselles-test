...
class _ForgetPasswordScreenState extends State<ForgetPasswordScreen> {
  ...
  bool get _isEmailMode => config.forgotPasswordVerification == 'email';
            ...
            builder: (context, splashProvider, _) {
              return Form(
                ...
                child: ListView(padding: const EdgeInsets.all(Dimensions.paddingSizeDefault), children: [
                  ...
                  Text(
                    _isEmailMode
                      ? getTranslated('enter_email_for_password_reset', context) ?? 'Enter your email address to reset your password'
                      : getTranslated('enter_phone_number_for_password_reset', context)!,
                      ...
                      if(forgetFormKey.currentState?.validate() ?? false) {
                        if(!(config.emailVerification ?? false) && !(config.phoneVerification ?? false) && config.customerVerification?.phone == 0 && config.customerVerification?.firebase == 0 && config.customerVerification?.email == 0) {
                          ...
                          if (!userInput.contains('@')) {
                            showCustomSnackBarWidget(getTranslated('enter_valid_email', context) ?? 'Please enter a valid email address', context, snackBarType: SnackBarType.warning);
                            return;
                          }

                          ResponseModel? response = await authProvider.forgetPassword(config: configModel, phoneOrEmail: userInput, type: 'email');
