...
class AuthController with ChangeNotifier {
  ...
  Future<void> socialLogin(SocialLoginModel socialLogin, Function callback, String? fromPage, VoidCallback? onLoginSuccess) async {
    _isLoading = true;
    notifyListeners();
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      if(token != null && token.isNotEmpty) {
        ...
        await sendVerificationCode(
          ...
          SignUpModel(email: userInputData, phone: userInputData), type: type0, fromPage: fromPage,
  ...
  Future<void> sendVerificationCode(ConfigModel config, SignUpModel signUpModel, {String? type, FromPage? fromPage, bool isResend = false, String? toNavigateScreen, VoidCallback? onLoginSuccess}) async {
    _resendButtonLoading = true;
    _isPhoneNumberVerificationButtonLoading = false;
    notifyListeners();
    if(config.customerVerification!.status == 1) {
      if(type == 'email' && config.customerVerification?.email == 1){
        await checkEmail(signUpModel.email!, fromPage, toNavigateScreen: toNavigateScreen, onLoginSuccess: onLoginSuccess);
      }else if(type == 'phone' && config.customerVerification?.firebase == 1){
        await firebaseVerifyPhoneNumber(signUpModel.phone!, fromPage!, isResend: isResend, toNavigateScreen: toNavigateScreen, onLoginSuccess: onLoginSuccess);
      }else if(type == 'phone' &&  config.customerVerification?.phone == 1){
        await checkPhone(signUpModel.phone!, fromPage, toNavigateScreen: toNavigateScreen, onLoginSuccess: onLoginSuccess);
      }
    }
  ...
  Future<ResponseModel> checkEmail(String email, FromPage? fromPage, {String? toNavigateScreen, VoidCallback? onLoginSuccess}) async {
    _isPhoneNumberVerificationButtonLoading = true;
    _resendButtonLoading = true;
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      showCustomSnackBarWidget(_verificationMsg, Get.context!, snackBarType: SnackBarType.error);
  ...
  Future<void> firebaseVerifyPhoneNumber(String phoneNumber, FromPage fromPage, {bool isForgetPassword = false, bool isResend = false, String? toNavigateScreen, VoidCallback? onLoginSuccess}) async {
    if(!isResend) {
      _isPhoneNumberVerificationButtonLoading = true;
    }
    _resendButtonLoading = true;
    ...
    await FirebaseAuth.instance.verifyPhoneNumber(
      ...
      verificationFailed: (FirebaseAuthException e) {
        ...
        if(e.code == 'invalid-phone-number') {
          showCustomSnackBarWidget(getTranslated('please_submit_a_valid_phone_number', Get.context!), Get.context!, snackBarType: SnackBarType.warning);
        }else{
          showCustomSnackBarWidget(getTranslated('${e.message}'.replaceAll('_', ' ').toCapitalized(),  Get.context!), Get.context!, snackBarType: SnackBarType.warning);
        }
      ...
      codeSent: (String vId, int? resendToken) async {
        ...
        if(fromPage == FromPage.verification) {
          showCustomSnackBarWidget(getTranslated('resend_code_successful', Get.context!), Get.context!, snackBarType: SnackBarType.success);
        }
      ...
      codeAutoRetrievalTimeout: (String verificationId) {
        ...
        showCustomSnackBarWidget('Timed out waiting for SMS.', Get.context!, snackBarType: SnackBarType.error);
        notifyListeners();
      },
  ...
  Future<ResponseModel> checkPhoneForOtp(String phone, FromPage fromPage, {String? toNavigateScreen, VoidCallback? onLoginSuccess}) async {
    _isPhoneNumberVerificationButtonLoading = true;
    _verificationMsg = '';
    notifyListeners();
  ...
  Future<ResponseModel> checkPhone(String phone, FromPage? fromPage, {String? toNavigateScreen, VoidCallback? onLoginSuccess}) async {
    _isPhoneNumberVerificationButtonLoading = true;
    _verificationMsg = '';
    notifyListeners();
    ApiResponseModel apiResponse = await authServiceInterface.checkPhone(phone);
  ...
  Future<void> firebaseOtpLogin({required String phoneNumber, required String session, required String otp, bool isForgetPassword = false, String? toNavigateScreen, VoidCallback? onLoginSuccess}) async {
    _isPhoneNumberVerificationButtonLoading = true;
    notifyListeners();
  ...
  Future<(ResponseModel?, String?)> verifyPhoneForOtp(String phone) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      _verificationMsg = ApiChecker.getError(apiResponse).errors![0].message;
      showCustomSnackBarWidget(_verificationMsg, Get.context!, snackBarType: SnackBarType.warning);
      responseModel = ResponseModel(_verificationMsg, false);
    }
    _isPhoneNumberVerificationButtonLoading = false;
    notifyListeners();
  ...
  Future<(ResponseModel, String?)> registerWithSocialMedia (String name, {required String email, String? phone}) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      _loginErrorMessage = ApiChecker.getError(apiResponse).errors![0].message;
      showCustomSnackBarWidget(_loginErrorMessage, Get.context!, snackBarType: SnackBarType.warning);
      responseModel = ResponseModel(_loginErrorMessage, false);
    }
    _isPhoneNumberVerificationButtonLoading = false;
    notifyListeners();
