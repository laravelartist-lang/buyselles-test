...
class AuthRepository implements AuthRepoInterface{
  ...
  Future<void> saveUserToken(String token) async {
    ...
    try {
      await _secureStorage.write(key: AppConstants.userLoginToken, value: token);
      _cachedToken = token;
    } catch (e) {
      rethrow;
    }
  ...
  String getUserToken() {
    ...
    _cachedToken = sharedPreferences!.getString(AppConstants.userLoginToken) ?? '';
    return _cachedToken;
  }
  ...
  Future<void> saveGuestId(String guestId) async {
    ...
    _guestIdLoaded = true;
    try {
      await _secureStorage.write(key: AppConstants.guestId, value: guestId);
    } catch (e) {
      rethrow;
    }
  }
  ...
  String? getGuestIdToken() {
    if (_guestIdLoaded) return _cachedGuestId ?? '1';
    _cachedGuestId = sharedPreferences!.getString(AppConstants.guestId);
    _guestIdLoaded = true;
    return _cachedGuestId ?? '1';
  }
  ...
  Future<bool> clearGuestId() async {
    _cachedGuestId = null;
    _guestIdLoaded = false;
    sharedPreferences!.remove(AppConstants.guestId);
    await _secureStorage.delete(key: AppConstants.guestId);
    return true;
  }
  ...
  @override
  bool isLoggedIn() {
    return _cachedToken.isNotEmpty || sharedPreferences!.containsKey(AppConstants.userLoginToken);
  }
  ...
  Future<bool> clearSharedData() async {
    ...
    sharedPreferences?.remove(AppConstants.guestId);
    await _secureStorage.delete(key: AppConstants.userLoginToken);
    await _secureStorage.delete(key: AppConstants.guestId);
    return true;
  }
  ...
  Future<void> saveUserEmailAndPassword(String userData) async {
    try {
      await _secureStorage.write(key: AppConstants.userLogData, value: userData);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<void> setAppleLoginEmail(String email) async {
    try {
      await _secureStorage.write(key: AppConstants.appleLoginEmail, value: email);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<void> saveGuestCartId(String guestId) async {
    try {
      await _secureStorage.write(key: AppConstants.guestCartId, value: guestId);
    } catch (e) {
      rethrow;
    }
  }
