                    ...
                    children: [
                      Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                          ...
                          child: Text(
                            '${startingPrice != null ?
                                PriceConverter.convertPrice(
                                  context,
                                  startingPrice * (details.quantity ?? 1),
                            ...
                            '${endingPrice != null
                                ? ' - ${PriceConverter.convertPrice(
                                      context,
                                      endingPrice * (details.quantity ?? 1),
                                      discount: (productModel?.clearanceSale?.discountAmount ?? 0) > 0
                                          ? productModel?.clearanceSale?.discountAmount
                                          : productModel?.discount,
                                      discountType: (productModel?.clearanceSale?.discountAmount ?? 0) > 0
                        ...
                        if((productModel!.discount != null && productModel!.discount! > 0) || (productModel!.clearanceSale != null && productModel!.clearanceSale!.discountAmount! > 0) )...[
                          ...
                          CustomDirectionalityWidget(
                            child: Text('${PriceConverter.convertPrice(context, startingPrice! * (details.quantity ?? 1))}'
                                '${endingPrice!= null ? ' - ${PriceConverter.convertPrice(context, endingPrice * (details.quantity ?? 1))}' : ''}',
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      InkWell(
                        onTap: () {
                          if (details.quantity! > 1) {
                            int minQty = productModel!.minimumOrderQty ?? 1;
                            if (details.quantity! > minQty) {
                              details.setQuantity(details.quantity! - 1);
                            } else {
                              showCustomSnackBarWidget(
                                '${getTranslated('minimum_quantity_is', context)} $minQty',
                                context,
                      ...
                      Text(
                        '${details.quantity}',
                        style: titilliumBold.copyWith(fontSize: 22, color: Theme.of(context).primaryColor),
                      ),
                      InkWell(
                        onTap: () {
                          int stock = productModel!.currentStock ?? 0;
                          bool isDigital = productModel!.productType == 'digital';
                          bool hasSupplierMapping = (productModel!.hasActiveSupplierMapping ?? 0) == 1;
                          if (details.quantity! >= stock && !isDigital && !hasSupplierMapping) {
                            showCustomSnackBarWidget(getTranslated('out_of_stock', context), context, snackBarType: SnackBarType.warning);
                          } else if (details.quantity! < stock || isDigital || hasSupplierMapping) {
                            details.setQuantity(details.quantity! + 1);
                          }
                        },
                        child: Padding(
