...
$(document).on('change', '.product-list-filter .real-time-action-update', function () {
    ...
    let categoryIds = $('.product-list-filter [name="category_ids[]"]:checked').map(function() {
        return $(this).val();
    }).get();
    ...
    let pageTitleContentElement = $('#page-title-content');
    if (pageTitleContentElement) {
        pageTitleContentElement.text(pageTitleContentElement.data('default-title'));
    }
});
